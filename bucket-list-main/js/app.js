...
class BucketListApp {
    ...
    createBucketItemHTML(item) {
        ...
        const completedCard = item.completed ? 'bucket-item--done' : '';

        return `
            <div class="bucket-item ${completedCard} bg-white rounded-2xl shadow-md p-5 flex items-center gap-4">
                <!-- 체크박스 -->
                <button
                    class="flex-shrink-0 w-7 h-7 rounded-full border-2 flex items-center justify-center ${checkboxClass} transition-all hover:scale-110"
                ...
                <div class="flex gap-2 flex-shrink-0">
                    <button
                        class="action-btn px-4 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-all text-sm font-medium"
                        onclick="app.openEditModal('${item.id}', '${this.escapeHtml(item.title).replace(/'/g, "\\'")}')"
                    >
                        수정
                    </button>
                    <button
                        class="action-btn px-4 py-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-all text-sm font-medium"
